...
def text_to_vector(text, tokenizer, model):
    ...
    for key in tokenized.keys():
        tokenized[key] = tokenized[key].to(model.device)
    with torch.no_grad():
        model_output = model(**tokenized)
    sentence_embedding = _mean_pooling(model_output, tokenized['attention_mask'])
...
def process_text_data(ratings, users, books, tokenizer, model, vector_name, vector_size, vector_create=False):
    """
    Parameters
    ----------
    users : pd.DataFrame
    ...
    if vector_create:
        ...
        np.save(f'./data/text_vector/book_summary_{vector_name}', book_summary_vector_list)        
        ...
        for books_read in tqdm(users_['books_read']):
            if not isinstance(books_read, list) and pd.isna(books_read):  # 유저가 읽은 책이 없는 경우, 텍스트 임베딩을 0으로 처리
                user_summary_merge_vector_list.append(np.zeros((vector_size)))
                continue
        ...
        np.save(f'./data/text_vector/user_summary_merge_{vector_name}', user_summary_merge_vector_list)        
        
    else:
        print('Check Vectorizer')
        print('Vector Load')
        book_summary_vector_list = np.load(f'./data/text_vector/book_summary_{vector_name}', allow_pickle=True)
        user_summary_merge_vector_list = np.load(f'./data/text_vector/user_summary_merge_{vector_name}', allow_pickle=True)
...
def text_data_load(args):
    ...
    users_, books_ = process_text_data(train, users, books, tokenizer, model, args.model_args[args.model].vector_name, args.model_args[args.model].word_dim, args.model_args[args.model].vector_create)

    book_drop_columns = list(set(books_.columns) & set(books_context_.columns))
    book_drop_columns.remove('isbn')
    books_ = pd.merge(books_, books_context_.drop(book_drop_columns, axis=1), on='isbn', how='left')

    user_drop_columns = list(set(users_.columns) & set(users_context_.columns))
    user_drop_columns.remove('user_id')
    users_ = pd.merge(users_, users_context_.drop(user_drop_columns, axis=1), on='user_id', how='left')
